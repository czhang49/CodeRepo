...
function [BestCase,mistakes]=Problem2(ParamSet,R)
...
%% This to use the best case to calculate W
mistakes=zeros(1,5);

[labels_pool,data_pool]=gen(l,m,n,number,noise);
[~,~,mistakes(1,1)]=TrainPercepForW(labels_pool,data_pool,R);
[~,~,mistakes(1,2)]=TrainPercepWithMarForW(labels_pool,data_pool,R,B_percepwithmar_eta);
[~,~,mistakes(1,3)]=TrainWinnowForW(labels_pool,data_pool,R,B_winnow_alpha);
[~,~,mistakes(1,4)]=TrainWinnowWithMarForW(labels_pool,data_pool,R,B_winnowwithmar_alpha,B_winnowwithmar_gamma);
[~,~,mistakes(1,5)]=TrainAdaGradForW(labels_pool,data_pool,R,B_adagrad_eta);
