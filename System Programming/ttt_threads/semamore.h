 ...
 *  You may change min_val/max_val to be whatever you want.
 *  For grading purposes, you can assume max_val won't equal 0.
 */

/**
 * The struct for a Semamore.
 * It contains the value the Semaphore is initially, as well as its maximum value (before it should block).
...
// Blocks when value is at max_val, then increments the value once it is not at max_val.
void semm_post(Semamore *s);

// Cleans up associated memory with the struct, but does not free the struct itself.
