...
#else
#define LOCK_IF_SANITIZED do{}while(0)
#define UNLOCK_IF_SANITIZED do{}while(0)
#endif
...
void threadStatusSetInternal(const char *description,
  ...
  if (src_filename == NULL) src_filename = "(null)";
...
static void initThreadArray() {
  if (status_array == NULL) {
    ...
    status_array = (ThreadStatus*) 
      malloc(sizeof(ThreadStatus) * status_array_cap);
...
static int addThread(pthread_t new_thread) {
  ...
  if (status_array_cap == thread_count) {
    status_array_cap *= 2;
    status_array = (ThreadStatus*) 
      realloc(status_array, 
	      sizeof(ThreadStatus) * status_array_cap);
  }
...
int ts_pthread_create(pthread_t *thread, const pthread_attr_t *attr,
  ...
  if (thread) *thread = child_thread_id;
...
int ts_pthread_mutex_lock(pthread_mutex_t *mutex, const char *src_filename,
  ...
  if (tid >= 0) restoreStatus(tid, saved);
  return result;
}
