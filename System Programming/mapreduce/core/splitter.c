...
/** Split the lines in a file into parts with approximately the same
    ...
    For example, to split into 4 chunks:
      ./split_lines myfile 4 0 > myfile.chunk0
      ./split_lines myfile 4 1 > myfile.chunk1
      ./split_lines myfile 4 2 > myfile.chunk2
      ./split_lines myfile 4 3 > myfile.chunk3
...
void getSubrange(size_t length, int count, int idx,
...
void printHelp() {
  fprintf(stderr, "\n  split_lines <input_file> <count> <index>\n\n"
          ...
          "    split_lines myfile 3 0 > myfile.0\n"
          "    split_lines myfile 3 1 > myfile.1\n"
          "    split_lines myfile 3 2 > myfile.2\n\n");
  exit(1);
}
...
void write_portion(const char *data, size_t length, int count, int index) {
  ...
  if (end_ptr > start_ptr) {
    ...
    if (bytes_written != (size_t) (end_ptr - start_ptr)) {
      fprintf(stderr, "split_lines error: tried to write %llu bytes, wrote %llu\n", 
