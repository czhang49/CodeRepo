...
static datastore_entry_t *malloc_entry_t(const char *key, const char *value) {
  datastore_entry_t *entry = (datastore_entry_t *) malloc(sizeof(datastore_entry_t));
  entry->key = key;
  entry->value = value;
...
void datastore_iterate(datastore_t *ds, datastore_iterfun f) {
