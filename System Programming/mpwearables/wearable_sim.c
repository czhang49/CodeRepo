...
int get_end_timestamp(int e, StatsInfo* info) {
    ...
    for (i = 0;i < info->wearable_size_;i ++) {
        ...
        for (k = 0;cur->data_size_;k ++) {
            if (k != cur->data_size_ - 1 && cur->start_sleep_ + cur->sample_interval_ * k < (unsigned int)e)
...
void* stats_thread(void* arg) {
    ...
    for (i = 0;i < sample_size * 3;i += 3) { //Loop through the statitic requests...
        ...
        set_alarm(get_end_timestamp(t2, info)); //set an alarm to fire after we should have received our data
...
int main(int argc, const char* argv[]) {
    ...
    //create wearables
    Wearable** wearables = get_wearables(argv[3], &count, &sample_times, &sample_count, &latest_launch);
