...
BoundaryC= DirichletBC(V,u1,u0_boundary)
u=TrialFunction(V)
v=TestFunction(V)
a=inner(nabla_grad(u),nabla_grad(v))*dx
L=f*v*dx
Exact_U=Function(V)
solve(a == L,Exact_U,BoundaryC)

plot(Exact_U,mesh,interactive=True)
# mesh refinement
mesh = UnitSquare(4,4)
h = []
eL2 = []
...
for i in range(nmeshes):
    ...
    L=f*v*dx
    u=Function(V)
    solve(a == L,u,BoundaryC)
    errl2 = errornorm(Exact_U, u, norm_type='l2', degree_rise=3, mesh=mesh)
    errh1 = errornorm(Exact_U, u, norm_type='h1', degree_rise=3, mesh=mesh)
    eL2.append(errl2)
    eH1.append(errh1)
    h.append(mesh.hmax())
