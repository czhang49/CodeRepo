...
void testConstructor(size_t count);
void testPop(size_t count);
void testPush(size_t count);
...
vector<int> setUpVector(int seed, size_t count)
{
    Random rand_gen(seed);
    vector<int> result;
    result.reserve(seed);
    for (int i = 0; static_cast<size_t>(i) < count; i++)
        result.push_back(i);
...
void testPop(size_t count)
{
    ...
    for (size_t i = 0; i < to_insert.size(); i++)
    {
        ...
        if (count < 64 || static_cast<int>(i) != min)
        {
            printAfter(min, "pop");
            cout << myHeap;
        }
        if (static_cast<int>(i) != min)
        {
...
void testConstructor(size_t count)
{
    ...
    for (size_t i = 0; i < count; i++)
        myArray.push_back(static_cast<int>(i));
