...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good())
    {
         ...
         */
	string nameurl ;
	nameurl.append(ll.customer);
	nameurl.append(ll.url);
	whenVisitedTable.remove(nameurl);
	whenVisitedTable.insert(nameurl,ll.date);

	pageVisitedTable.remove(ll.url);
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
     ...
     */
	string nameurl ;
	nameurl.append(customer);
	nameurl.append(url);
	if (!whenVisitedTable.contains(nameurl)){
		return false;
	}
                                  ...
                                  const string& url) const
{
	...
	if (whenVisitedTable.contains(nameurl)){
		...
		time_t  time_recent= whenVisitedTable.at(nameurl);
		return time_recent;
	}
