...
if __name__ == '__main__':
	...
	for k in range (1,6):
		for j in range (1,6):
			content_path ='./examples/content_' + str (k) + '.jpg'
			style_path = './examples/style_' + str (j) + '.jpg'
		...
		#######################################################
			...
			#contentLayerNames = ['conv1_1']
				...
				resultImage = stylize(contentImage, styleImage, contentLayerNames = contentLayerNames, styleLayerNames = styleLayerNames)
				resultImage = resultImage[0,:,:,:]
				img = np.clip(resultImage, 0, 255).astype(np.uint8)
				output_path = './outputs/content' + str(k) '_style' + str(j) '_CLconv' + str(i) + '1_SLconv11conv21conv31conv41conv51.jpg'
